...
<html:form action="tenderProjectManage.do" onsubmit="return checkFormValidate();">
		...
		<table class="win" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" border="0">			
			...
			<tr>
				...
				<td>
					<html:text styleId="personName" property="tenderInfo.currentProjectTeam.account.person.personName" size="18" maxlength="100" readonly="true" style="background-color:LightGray;"/>*
					<html:button property="" onclick="openDialog('tenderProjectManage.do?method=searchAccount&type=teamMember',800,600);">...</html:button>
				</td>
				<td><bean:message bundle="projectmanage" key="member.projectrole" />：</td>
				<td>
					<html:select styleId="tenderRole" property="tenderInfo.currentProjectTeam.ptTenderRole" style="width:120">						
						<html:options collection="tenderRoleList" property="trId" labelProperty="trName"/>
					</html:select>*				
				</td>
				<td><bean:message bundle="projectmanage" key="feetax.remark" />：</td>
				<td>
					<html:text property="tenderInfo.currentProjectTeam.ptRemark" size="30" maxlength="100"/>*
				</td>	
						...
						<td align="center">
							<logic:equal name="accountPerson" property="hasCreatePower" value="true">
								<logic:notEmpty name="tm" property="ptId">
									<a href='javascript:if(confirm("确认要删除这条信息吗?")) {chgAction(document.all.id,"<bean:write name="tm" property="ptId"/>");chgAction(document.all.method,"deleteProjectTeamMember");tenderProjectForm.submit();}'><img border="0" src="pages\images\icon\16x16\delete.gif"></a>
								</logic:notEmpty>
								<logic:empty name="tm" property="ptId">
									<a href='tenderProjectManage.do?method=deleteProjectTeamMemberByIndex&index=<%=i %>'><img border="0" src="pages\images\icon\16x16\delete.gif"></a>
								</logic:empty>
							</logic:equal>
						</td>					
					</tr>
