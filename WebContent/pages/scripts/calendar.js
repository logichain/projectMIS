...
{
    var date = new Date();
    var by = date.getFullYear()-60; //最小值 → 60 年前
...
function SelectDateFormat(obj,dateFormatStyle)
{
    var date = new Date();
    var by = date.getFullYear()-60; //最小值 → 60 年前
    var ey = date.getFullYear()+5; //最大值 → 5 年后
    cal = (cal==null) ? new Calendar(by, ey, 0) : cal;    //初始化为中文版，1为英文版
    cal.dateFormatStyle = dateFormatStyle; // 默认显示格式为:yyyy-MM-dd ,还可显示 yyyy/MM/dd
    cal.show(obj);
}
/**//* 返回日期 */
...
if(this.dateFormatStyle.length == 7)
{
	this.date = (dateObj.value.length > 0) ? new Date((dateObj.value + "-01").toDate(this.dateFormatStyle + "-dd")) : new Date() ;//若为空则显示当前月份
}
else
{
...
if(xy.x + 150 > clientwidth)
{
	this.panel.style.left = clientwidth -150 + "px";
}
else
{
...
if(xy.y + dateObj.offsetHeight + 200 > clientheight)
{
	this.panel.style.top = clientheight -200 + "px";
}
else
{
